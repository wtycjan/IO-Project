class Functions(object):
    ...
    @classmethod
    def checking_connections_between_functions(cls, file_list):  # funkcja sprawdzająca jak funkcje są połączone z sobą
        try:
            functions_list = []
                ...
                with open(actually_file, 'r') as f:
                    for line in f:
                        ...
                        if split_text and split_text[0] == "def":
                            ...
                            if tmp[0] != "__init__":
                                Functions.functionsConnectionList.append(tmp[0])
            for el in range(int(len(Functions.functionsConnectionList)/2)):
                Functions.functionsConnectionWeight.append(str(0))
